...
if  [ "$jail" == "false" ]; then
curl -s -X POST $URL -d "chat_id=$ID_CHAT" -d "parse_mode=html" -d "text= ✅ $VALIDATOR IS FINE"

elif [ "$jail" == "true" ]; then
 curl -s -X POST $URL -d "chat_id=$ID_CHAT" -d "parse_mode=html" -d "text= ❌ $VALIDATOR IS JAILED"
fi
block=$(haqqd q slashing signing-info $(haqqd tendermint show-validator) -oj | jq .missed_blocks_counter | grep -o -E '[0-9]+')
if  [[ "$block" -eq 0 ]]; then
  curl -s -X POST $URL -d "chat_id=$ID_CHAT" -d "parse_mode=html" -d "text= ✅ NODE MISSED NO BLOCK"

elif [[ "$block" -gt 0 ]]; then
 curl -s -X POST $URL -d "chat_id=$ID_CHAT" -d "parse_mode=html" -d "text= ⚠️ NODE IS MISSING BLOCKS: $block missed blocks"
fi
   printf "sleep"
